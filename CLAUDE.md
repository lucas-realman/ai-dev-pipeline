...
当用户说 **"评审"** 时，必须执行以下**完整两阶段评审流程**（缺一不可）：

1. 读取 `docs/12-review/README.md` 获取角色矩阵与技术评审维度
2. 确定新版本号，创建 `docs/12-review/vX.Y/`
3. **阶段一 — 8 角色评审**: 为每个角色生成独立评审意见文件 (8 份)
4. **阶段二 — 4 维技术专项评审**: 生成以下 4 份专项评审报告
   - `09-技术实现可行性评审.md` — 逐模块评估实现完整度、算法正确性、异常健壮性
   - `10-接口可行性评审.md` — 逐接口评估参数一致性、协议适配、错误处理
   - `11-技术方案合理性评审.md` — 架构决策合理性、技术成熟度、可扩展性
   - `12-风险与缓解措施.md` — 技术风险识别 + 缓解方案，按 P0~P3 分级
5. 生成 `评审总结.md` 汇总所有角色意见与技术审查结论
6. 生成 `ACTION-ITEMS.md` 行动项跟踪 (两阶段合并去重)
7. 更新 `docs/12-review/README.md` 版本记录
8. **打 Git Tag**: `git tag -a review-vX.Y -m "评审 vX.Y: {范围} — {结论}"` 并推送

> **强制要求**: 每次评审必须同时产出阶段一 (8 份) + 阶段二 (4 份) + 评审总结 + ACTION-ITEMS = 共 14 份文件。
